...
1a. Download the JSON credentials and save them into a configuration directory within you project.

1b. Set the path to your credentials as below:
    ```php
...
1c. Set the scopes required for the API you are going to call. 
    ```php
    $client->addScope('shipping');
    ```

1d. Set your application's redirect URI. The redirect URI must match the authorised redirect URI 
you set when creating your client credentials.
    ```php
    $client->setRedirectUri('https://example.com/callback');
    ```

1e. Set a use-once state number. This number will be returned to your callback page so that you
...
1f. Get the Evance OAuth 2.0 URL for a user to authorise access. You can either use this URL in
redirect or provide a link for the user to click.
    ```php
    $url = $client->createAuthorizeUrl();
    ```
    
1g. In the callback script (your redirect URI), you will need to exchange the authorisation code
...
2. Create your App and credentials.

2a. Download the JSON credentials and save them into a configuration directory within you project.

2b. Set the path to your credentials as below:
    ```php
    use Evance\ApiClient;
    $client = new ApiClient();
...
2c. Optionally, you may set the scope you wish to grant the access token. Omitting this step
will grant the access token full access to the scopes granted to your App. 
    ```php
    $client->addScope('shipping');
    ```
    
2d. Now you can obtain the access token from Evance. 
    ```php
